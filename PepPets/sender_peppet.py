...
'''
IMPORTANT: For the MVP, one Pep Pet runs sender_peppet.py and completely disregards receiver_peppet.py
The sender Pep Pet continously writes their own ID to a serial port, and 
continously queries our database to see if new friends have been added. 
'''
...
class PepPet:
    ...
    def writeMessage(self):
        while not NIGHT:
            previous_friends = ""
            ser.write("Pep Pet 1 ID")
            time.sleep(5)

            query = "SELECT friends from ID_Emails WHERE petid='" + self.unique_id + "'"
            cursor.execute(query)
            record = cursor.fetchone()
            if record == previous_friends:
                # No new friend was found
                continue
            else: # New friend found! 
                found_friends = record.rsplit(',', 1)
                self.connectWithFriend(found_friends[1])
                previous_friends = record
                pass
...
Thread 6: Check for friends: For friend connections, continously write message to serial port and query database
'''
initpins(bar1)
initpins(bar2)
initpins(bar3)
